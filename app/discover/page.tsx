...
// There is no separate pattern-detection/opportunity/leakage endpoint in the
// backend (grepped server.js + lib/domain/intelligence/*.js in full —
// opportunityPropagation.js exists but is never wired to a route, so it is
// not a real, reachable capability today). Every signal already carries a
// real deterministic explanation (why_exists), a real evidence trail
// (api.intelligence.impact -> evidence[]), a real severity dot
// (status/impact_status), and a real confidence signal (plausibility_confidence).
// discovery_row below is intel_row's shape plus a "why matters" line
// (why_exists, same field Bridge already renders) and an evidence mark that
// opens the same EvidenceDrawer Bridge uses.
//
// Of the 7 subnav tabs (§14), only three have a genuine, honest predicate
// over this real data:
...
// Opportunities, Leakage, Patterns, and Saved have NO real backing: there is
// no upside/opportunity signal type, no margin-leakage computation, no
// cross-signal pattern-clustering output, and no save/bookmark endpoint
// anywhere in the backend. Those four tabs render the honest V32 empty-state
// pattern (§8) rather than forcing real signals into categories the data
// doesn't support.
...
export default function DiscoverPage() {
  ...
  return (
    <DashboardLayout pageTitle="Discover">
      ...
      {!loading && hasRealBacking && visible.length === 0 && (
