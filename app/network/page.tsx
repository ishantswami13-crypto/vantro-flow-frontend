  ...
  return (
    <div className="relative shrink-0" style={{ width: size, height: size }}>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} className="-rotate-90">
        <circle cx={size/2} cy={size/2} r={r} fill="none" stroke="#E5E5E1" strokeWidth="4" />
    ...
    <DashboardLayout pageTitle="Starlane Network">
      <div className="max-w-4xl mx-auto space-y-5 page-enter">
        ...
        <div className="card-premium p-4 space-y-3">
          <div className="flex gap-2">
            ...
            <button onClick={search} disabled={loading}
              className="px-4 py-2.5 rounded-xl bg-gray-900 text-white text-sm font-semibold hover:bg-gray-800 transition-all shadow-button-accent disabled:opacity-50 shrink-0">
            ...
            {BUSINESS_TYPES.map(({ key, label, icon: Icon }) => (
              <button key={key} onClick={() => { setFilter(key); }}
                className={[
                  ...
                  filter === key
                    ? "bg-gray-900 text-white border-accent shadow-button-accent"
        ...
        <div className="card-premium p-5 flex items-center gap-4">
          ...
          <Link href="/my-id"
            className="shrink-0 px-4 py-2 rounded-xl bg-gray-900 text-white text-xs font-bold hover:bg-gray-800 transition-all shadow-button-accent whitespace-nowrap">
            Share My ID →
          </Link>
        </div>
