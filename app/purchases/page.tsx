...
import { FiPlus, FiEdit2, FiTrash2, FiAlertCircle, FiCheckCircle, FiClock } from "react-icons/fi";
...
export default function PurchasesPage() {
  ...
  const emptyForm = { supplier_name: "", supplier_phone: "", bill_number: "", purchase_date: new Date().toISOString().split("T")[0], due_date: "", total_amount: "", paid_amount: "0", notes: "" };
  ...
  const save = async () => {
    ...
    try {
      ...
      const r = await fetch(url, {
        ...
        body: JSON.stringify({ ...form, total_amount: parseFloat(form.total_amount), paid_amount: parseFloat(form.paid_amount || "0") }),
      });
      if (r.ok) { setShowAdd(false); setEditId(null); setForm(emptyForm); load(); }
    ...
    <div className="p-4 max-w-4xl mx-auto pb-24">
      ...
      {showAdd && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-md bg-surface-1 rounded-2xl border border-white/10 overflow-hidden max-h-[90vh] overflow-y-auto">
            <div className="px-5 py-4 border-b border-white/5">
              <h3 className="font-bold text-primary">{editId ? "Edit Purchase" : "Add Purchase"}</h3>
              <p className="text-xs text-muted">Supplier ka bill add karo</p>
            </div>
            <div className="p-5 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input value={form.purchase_date} onChange={e => setForm(f => ({ ...f, purchase_date: e.target.value }))}
                    type="date"
                    className="w-full bg-surface-2 border border-white/8 rounded-xl px-3 py-2.5 text-sm text-primary focus:outline-none focus:border-accent/50" />
                </div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input value={form.paid_amount} onChange={e => setForm(f => ({ ...f, paid_amount: e.target.value }))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="w-full max-w-sm bg-surface-1 rounded-2xl border border-white/10 p-5">
            ...
            <div className="flex gap-3">
              <button onClick={() => setPayModal(null)} className="flex-1 py-2.5 rounded-xl bg-surface-2 text-secondary text-sm font-semibold">Cancel</button>
