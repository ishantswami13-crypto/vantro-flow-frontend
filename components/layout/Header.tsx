...
const NOTIF_ICON: Record<string, { icon: typeof FiCheck; color: string }> = {
  payment:  { icon: FiCheck,       color: "#1A8F5C" },
  overdue:  { icon: FiAlertCircle, color: "#C13B3B" },
  forecast: { icon: FiTrendingUp,  color: "#171717" },
  system:   { icon: FiBell,        color: "#8A8A86" },
};
      ...
      <div className="flex items-center gap-1 shrink-0">
        ...
        <div className="relative" ref={panelRef}>
          <button
            ...
            style={{ color: showNotifs ? "#171717" : "#8A8A86", background: showNotifs ? "rgba(20,20,20,0.05)" : "transparent" }}
            aria-label="Notifications"
            onMouseEnter={e => { if (!showNotifs) (e.currentTarget as HTMLElement).style.background = "rgba(20,20,20,0.05)"; }}
            onMouseLeave={e => { if (!showNotifs) (e.currentTarget as HTMLElement).style.background = "transparent"; }}
          >
            <FiBell size={15} />
            {unread > 0 && (
              <span className="absolute top-1.5 right-1.5 w-1.5 h-1.5 rounded-full border" style={{ background: "#C13B3B", borderColor: "#F7F7F4" }} />
            )}
          </button>
          ...
          {showNotifs && (
            <div
              ...
              style={{ background: "#FFFFFF", border: "1px solid #E5E5E1", boxShadow: "0 16px 48px rgba(0,0,0,0.14)" }}
            >
              <div className="flex items-center justify-between px-4 py-3" style={{ borderBottom: "1px solid #EDEDE9" }}>
                <div className="flex items-center gap-2">
                  <p className="text-[12px] font-medium" style={{ color: "#171717" }}>Notifications</p>
                  {unread > 0 && (
                    <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full" style={{ background: "#C13B3B", color: "#fff" }}>
                      {unread}
                    </span>
                  )}
                </div>
                <button onClick={() => setShowNotifs(false)} style={{ color: "#8A8A86" }}>
                  <FiX size={13} />
                </button>
              </div>
              ...
              <div className="max-h-72 overflow-y-auto">
                {loadingNotifs ? (
                  <div className="py-8 text-center">
                    <div className="w-4 h-4 border-2 border-t-transparent rounded-full animate-spin mx-auto mb-2" style={{ borderColor: "#E5E5E1", borderTopColor: "transparent" }} />
                    <p className="text-[11px]" style={{ color: "#8A8A86" }}>Loading…</p>
                  </div>
                ) : notifs.length === 0 ? (
                  <div className="py-8 text-center px-4">
                    <FiBell size={20} className="mx-auto mb-3" style={{ color: "#D8D8D3" }} />
                    <p className="text-[12px] font-medium mb-1" style={{ color: "#686868" }}>No notifications</p>
                    <p className="text-[11px]" style={{ color: "#8A8A86" }}>Payments and alerts will appear here.</p>
                  </div>
                ) : (
                  <div>
                    {notifs.map((n, i) => {
                      ...
                      return (
                        <div
                          ...
                          style={{ borderBottom: i < notifs.length - 1 ? "1px solid #F2F2EF" : "none", background: !n.read ? "#FAFAF8" : "transparent" }}
                        >
                          <div className="w-6 h-6 rounded-lg flex items-center justify-center shrink-0 mt-0.5" style={{ background: `${color}14` }}>
                            <Icon size={12} style={{ color }} />
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className="text-[11px] font-medium leading-tight" style={{ color: "#171717" }}>{n.title}</p>
                            <p className="text-[10px] mt-0.5 leading-relaxed" style={{ color: "#8A8A86" }}>{n.body}</p>
                            <p className="text-[10px] mt-1" style={{ color: "#B5B5B0" }}>{timeAgo(n.time)}</p>
                          </div>
                          {!n.read && <div className="w-1.5 h-1.5 rounded-full shrink-0 mt-1.5" style={{ background: "#171717" }} />}
                        </div>
                      );
                    })}
                  </div>
              ...
              <div className="px-4 py-2.5 flex items-center justify-between" style={{ borderTop: "1px solid #EDEDE9" }}>
                <button
                  onClick={fetchNotifs}
                  className="text-[11px] transition-colors"
                  style={{ color: "#8A8A86" }}
                  onMouseEnter={e => ((e.currentTarget as HTMLElement).style.color = "#171717")}
                  onMouseLeave={e => ((e.currentTarget as HTMLElement).style.color = "#8A8A86")}
                >
                  Refresh
                </button>
                <Link
